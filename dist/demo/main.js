...
import { Grid, Direction } from '../systems/Grid';
...
const ROBOT_START = { x: GRID_CENTER_X, y: 0 };
const BUTTON_POS = { x: GRID_CENTER_X, y: 5 };
const CHARGE_POS = { x: GRID_CENTER_X, y: 11 };
const WALL_H = 3.0;
...
const dirNames = { 0: 'K', 90: 'D', 180: 'G', 270: 'B' };
let scene, camera, renderer, controls;
let robot, robotMesh, grid, battery, executor;
let button, door, chargingPad;
let doorPanels = [], doorLines = [], doorHandle;
let workspace;
let wheelMeshes = [], chargePadMesh;
let mainAmbientLight = null;
let chargingSurfaceMat = null;
let chargingAmbientGlowMat = null;
let isExecuting = false, isPanelOpen = true;
let commandBlockMap = [], highlightedBlockId = null;
let skyLight;
let doorClipPlane;
let doorGlowPlane = null;
let doorSunLight = null;
let doorAnimationVersion = 0;
let robotEyeMeshes = [];
let robotChargeRingMat = null;
let robotChargeBaseIntensity = 0.6;
let robotChargeBar = null;
let robotChargePort = null;
let chargingCableAnchor = null;
let chargingCableLine = null;
...
let garageRoofGroup = null;
let garageRoofMaterials = [];
var GameState;
(function (GameState) {
    GameState[GameState["INTRO"] = 0] = "INTRO";
    GameState[GameState["READY"] = 1] = "READY";
    GameState[GameState["EXECUTING"] = 2] = "EXECUTING";
    GameState[GameState["DOOR_OPENED"] = 3] = "DOOR_OPENED";
    GameState[GameState["COMPLETE"] = 4] = "COMPLETE";
    GameState[GameState["FAILED"] = 5] = "FAILED";
})(GameState || (GameState = {}));
let gameState = GameState.INTRO;
function getPrimaryCameraPose(mode = garageMode) {
    if (mode === 'closed') {
        return {
...
function applyGarageMode(mode, animate = false, notify = false) {
    garageMode = mode;
    const roofVisible = mode === 'closed';
    if (garageRoofGroup)
        garageRoofGroup.visible = roofVisible;
    ...
    if (camera && controls) {
        if (animate)
            animateCameraTo(pose.pos, pose.target, 900);
        else {
    ...
    if (notify) {
        showToast(mode === 'closed'
            ? 'Garaj modu: Kapalı tavan sürüş görünümü'
            : 'Garaj modu: Açık tavan sahne görünümü', 'info');
    }
}
// ═══════════════════════════════════════════
// UI HELPERS
...
function updateMission(num, title, desc) {
...
function showSuccess(msg, cmds, bat) {
...
function initScene() {
    ...
    moonFill.position.set(-5, 9, -4);
    moonFill.castShadow = true;
    moonFill.shadow.mapSize.set(2048, 2048);
    moonFill.shadow.camera.left = -12;
    moonFill.shadow.camera.right = 12;
    moonFill.shadow.camera.top = 12;
    moonFill.shadow.camera.bottom = -12;
    moonFill.shadow.bias = -0.00015;
    scene.add(moonFill);
...
// ROBOT
// ═══════════════════════════════════════════
function createRobot(pos) {
    const g = new THREE.Group();
    wheelMeshes = [];
    robotEyeMeshes = [];
    const body = new THREE.Mesh(new RoundedBoxGeometry(0.72, 0.62, 0.82, 4, 0.1), new THREE.MeshStandardMaterial({ color: 0x87CEEB, roughness: 0.2, metalness: 0.08 }));
    body.position.y = 0.42;
    body.castShadow = true;
    g.add(body);
    const panel = new THREE.Mesh(new RoundedBoxGeometry(0.74, 0.52, 0.1, 4, 0.06), new THREE.MeshStandardMaterial({ color: 0xffffff, roughness: 0.12 }));
    panel.position.set(0, 0.42, 0.46);
    g.add(panel);
    const screen = new THREE.Mesh(new RoundedBoxGeometry(0.52, 0.28, 0.03, 4, 0.02), new THREE.MeshStandardMaterial({ color: 0x111111, roughness: 0.1, metalness: 0.3 }));
    screen.position.set(0, 0.46, 0.52);
    g.add(screen);
    const chargeBarTrack = new THREE.Mesh(new RoundedBoxGeometry(0.36, 0.05, 0.012, 2, 0.006), new THREE.MeshStandardMaterial({ color: 0x20252f, roughness: 0.45, metalness: 0.4 }));
    chargeBarTrack.position.set(0, 0.36, 0.536);
    g.add(chargeBarTrack);
    robotChargeBar = new THREE.Mesh(new RoundedBoxGeometry(0.34, 0.036, 0.008, 2, 0.005), new THREE.MeshStandardMaterial({ color: 0xff7043, emissive: 0xff7043, emissiveIntensity: 1.2, roughness: 0.15 }));
    ...
    leftEye.position.set(-0.13, 0.46, 0.535);
    g.add(leftEye);
    const rightEye = new THREE.Mesh(eyeGeo, eyeMat);
    rightEye.position.set(0.13, 0.46, 0.535);
    g.add(rightEye);
    ...
    g.add(new THREE.PointLight(0xFFD700, 0.4, 1.5).translateX(0.13).translateY(0.46).translateZ(0.6));
    const smile = new THREE.Mesh(new RoundedBoxGeometry(0.16, 0.025, 0.02, 2, 0.01), new THREE.MeshStandardMaterial({ color: 0xFFD700, emissive: 0xFFD700, emissiveIntensity: 1 }));
    smile.position.set(0, 0.34, 0.535);
    g.add(smile);
    const top = new THREE.Mesh(new RoundedBoxGeometry(0.52, 0.16, 0.52, 4, 0.06), new THREE.MeshStandardMaterial({ color: 0x5B9BD5, roughness: 0.3 }));
    ...
    const ant = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.28, 16), new THREE.MeshStandardMaterial({ color: 0x87CEEB }));
    ant.position.y = 1.05;
    g.add(ant);
    const aBall = new THREE.Mesh(new THREE.SphereGeometry(0.065, 16, 16), new THREE.MeshStandardMaterial({ color: 0x00E5FF, emissive: 0x00E5FF, emissiveIntensity: 2.5 }));
    aBall.position.y = 1.22;
    g.add(aBall);
    ...
    const wPos = [[-0.37, 0.19, 0.36], [0.37, 0.19, 0.36], [-0.37, 0.19, -0.36], [0.37, 0.19, -0.36]];
    wPos.forEach(p => {
        ...
        r.position.set(p[0] > 0 ? p[0] + 0.085 : p[0] - 0.085, p[1], p[2]);
        g.add(r);
    });
...
// INTERACTABLES
// ═══════════════════════════════════════════
function createButton3D(pos) {
    const base = new THREE.Mesh(new THREE.CylinderGeometry(0.28, 0.28, 0.12, 32), new THREE.MeshStandardMaterial({ color: 0xc87800, roughness: 0.3, metalness: 0.3 }));
    base.position.set(pos.x, 0.06, pos.y);
    base.castShadow = true;
    scene.add(base);
    const btn = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 0.1, 32), new THREE.MeshStandardMaterial({ color: 0xFFEB3B, emissive: 0xFFD700, emissiveIntensity: 0.8, roughness: 0.2 }));
    btn.position.set(pos.x, 0.17, pos.y);
    btn.castShadow = true;
    scene.add(btn);
    const glow = new THREE.Mesh(new THREE.SphereGeometry(0.35, 16, 16), new THREE.MeshBasicMaterial({ color: 0xFFEB3B, transparent: true, opacity: 0.15 }));
    glow.position.set(pos.x, 0.2, pos.y);
    scene.add(glow);
    scene.add(new THREE.PointLight(0xFFD700, 0.6, 4).translateX(pos.x).translateY(0.5).translateZ(pos.y));
}
function createChargingStation(pos) {
    const g = new THREE.Group();
    ...
    g.add(platform);
    const dockingRing = new THREE.Mesh(new THREE.TorusGeometry(0.78, 0.065, 20, 56), new THREE.MeshStandardMaterial({ color: 0x4dd0e1, emissive: 0x26c6da, emissiveIntensity: 1.05, roughness: 0.2, metalness: 0.5 }));
    dockingRing.rotation.x = Math.PI / 2;
    dockingRing.position.y = 0.11;
    g.add(dockingRing);
    const centerPad = new THREE.Mesh(new THREE.CylinderGeometry(0.55, 0.62, 0.06, 48), new THREE.MeshStandardMaterial({ color: 0x1f2a33, roughness: 0.4, metalness: 0.45 }));
    ...
    const centerGlow = new THREE.Mesh(new THREE.CylinderGeometry(0.44, 0.48, 0.02, 40), chargingSurfaceMat);
    ...
    g.add(servicePanel);
    // Status display
    const statusBar = new THREE.Mesh(new RoundedBoxGeometry(0.4, 0.12, 0.015, 3, 0.01), new THREE.MeshStandardMaterial({ color: 0x69f0ae, emissive: 0x69f0ae, emissiveIntensity: 2.1, roughness: 0.08 }));
    statusBar.position.set(0, 1.52, -0.9);
    g.add(statusBar);
    const statusStripe = new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.03, 0.02), new THREE.MeshStandardMaterial({ color: 0x4dd0e1, emissive: 0x4dd0e1, emissiveIntensity: 1.4, roughness: 0.1 }));
    ...
    frameLeft.position.set(-1.0, 1.2, -0.1);
    frameLeft.castShadow = true;
    g.add(frameLeft);
    const frameRight = new THREE.Mesh(new THREE.BoxGeometry(0.18, 2.4, 0.18), frameMat);
    frameRight.position.set(1.0, 1.2, -0.1);
    frameRight.castShadow = true;
    g.add(frameRight);
    const frameTop = new THREE.Mesh(new THREE.BoxGeometry(2.18, 0.16, 0.2), frameMat);
    frameTop.position.set(0, 2.38, -0.1);
    frameTop.castShadow = true;
    g.add(frameTop);
    // Cable reel + connector dock
    const reel = new THREE.Mesh(new THREE.TorusGeometry(0.2, 0.05, 16, 32), new THREE.MeshStandardMaterial({ color: 0x20252d, roughness: 0.72, metalness: 0.45 }));
    reel.position.set(0.52, 0.8, -1.0);
    reel.rotation.y = Math.PI / 2;
    g.add(reel);
    const dockNozzle = new THREE.Mesh(new RoundedBoxGeometry(0.12, 0.18, 0.09, 3, 0.015), new THREE.MeshStandardMaterial({ color: 0x25303c, roughness: 0.28, metalness: 0.65 }));
    ...
    // Overhead canopy
    const canopy = new THREE.Mesh(new RoundedBoxGeometry(2.7, 0.12, 3.2, 5, 0.03), new THREE.MeshStandardMaterial({ color: 0x3b4a5e, roughness: 0.25, metalness: 0.72 }));
    canopy.position.y = 2.64;
    canopy.castShadow = true;
    g.add(canopy);
    const canopyLed = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.03, 0.05), new THREE.MeshStandardMaterial({ color: 0x7bdff2, emissive: 0x7bdff2, emissiveIntensity: 1.65, roughness: 0.08 }));
    ...
    if (chargingCableLine)
        scene.remove(chargingCableLine);
    ...
    chargingCableLine = new THREE.Line(cableGeometry, new THREE.LineBasicMaterial({ color: 0x16181f, transparent: true, opacity: 0.9 }));
...
function createDoor() {
    ...
    doorHandle = new THREE.Mesh(new RoundedBoxGeometry(0.1, 0.32, 0.12, 2, 0.02), new THREE.MeshStandardMaterial({
        color: 0x333340, roughness: 0.3, metalness: 0.8,
        clippingPlanes: [doorClipPlane], clipShadows: true
    }));
    doorHandle.position.set(GRID_CENTER_X - doorW * 0.33, 0.9, DOOR_Z + 0.1);
...
function animateDoorOpening() {
    ...
    if (doorSunLight)
        scene.remove(doorSunLight);
    doorSunLight = new THREE.DirectionalLight(0xfff4e0, 0);
    doorSunLight.position.set(GRID_CENTER_X + 2.5, 9, DOOR_ROW + 15);
    scene.add(doorSunLight);
    // Glow plane (will be removed after animation)
    if (doorGlowPlane)
        scene.remove(doorGlowPlane);
    ...
    doorGlowPlane.position.set(GRID_CENTER_X, WALL_H / 2, DOOR_ROW + 0.1);
    scene.add(doorGlowPlane);
    const animate = () => {
        if (animationVersion !== doorAnimationVersion)
            return;
        ...
        if (doorSunLight)
            doorSunLight.intensity = ease * 2.2;
        if (t < 0.8)
            glowMat.opacity = ease * 0.3;
        else
            glowMat.opacity = 0.3 * (1 - (t - 0.8) / 0.2); // fade out in last 20%
        renderer.toneMappingExposure = BASE_EXPOSURE + ease * DOOR_OPEN_EXPOSURE_BOOST;
        if (skyLight)
            skyLight.intensity = ease * 0.95;
        if (t < 1)
            ...
            for (let x = 0; x < GRID_W; x++)
                grid.setWalkable({ x, y: DOOR_ROW }, true);
...
function highlightCommandByIndex(index) {
...
function resetSimulationState() {
    ...
    for (let x = 0; x < GRID_W; x++)
        grid.setWalkable({ x, y: DOOR_ROW }, false);
...
// CAMERA
// ═══════════════════════════════════════════
function animateCameraTo(tp, tl, dur, cb) {
...
function updateRobotPosition() {
    ...
    const a = () => {
        ...
        const e = t < 0.5 ? 2 * t * t : 1 - Math.pow(-2 * t + 2, 2) / 2;
        robotMesh.position.x = sx + (tx - sx) * e;
        robotMesh.position.z = sz + (tz - sz) * e;
        let dr = tr - sr;
        while (dr > Math.PI)
            dr -= Math.PI * 2;
        while (dr < -Math.PI)
        ...
        if (t < 1)
            ...
            robotMesh.position.x = tx;
            robotMesh.position.z = tz;
            robotMesh.rotation.y = tr;
        }
    };
    a();
    updatePositionDisplay();
}
function isRobotOnChargingPad() {
    const pos = robot.getPosition();
    return pos.x === CHARGE_POS.x && pos.y === CHARGE_POS.y;
}
function setChargingCableConnected(connected) {
    isCableConnected = connected;
    if (connected)
        cableConnectStart = performance.now();
    if (chargingCableLine)
        chargingCableLine.visible = connected;
}
function updateChargingCableGeometry(start, end) {
    if (!chargingCableLine)
        return;
    ...
    for (let i = 0; i < 14; i++) {
        const t = i / 13;
        const omt = 1 - t;
        points.push(new THREE.Vector3(omt * omt * start.x + 2 * omt * t * mid.x + t * t * end.x, omt * omt * start.y + 2 * omt * t * mid.y + t * t * end.y, omt * omt * start.z + 2 * omt * t * mid.z + t * t * end.z));
    }
    chargingCableLine.geometry.setFromPoints(points);
}
function updateChargingCable() {
    if (!chargingCableLine || !chargingCableAnchor || !robotChargePort)
        return;
    if (!isCableConnected) {
        chargingCableLine.visible = false;
        return;
...
function updateRobotChargeIndicator(level) {
    ...
    if (robotChargeBar) {
        const barMat = robotChargeBar.material;
...
function initGame() {
    ...
    for (let x = 0; x < GRID_W; x++)
        grid.setWalkable({ x, y: DOOR_ROW }, false);
    // Events
    EventBus.on('battery:updated', (l) => { updateBatteryUI(l); updateRobotChargeIndicator(l); });
    ...
    EventBus.on('command:highlight', (index) => highlightCommandByIndex(index));
    EventBus.on('command:executed', (d) => {
        switch (d.type) {
            case 'MOVE_FORWARD':
            case 'MOVE_BACKWARD':
    ...
    EventBus.on('battery:full', () => {
        ...
        chargePulseUntil = 0;
        updateBatteryUI(100);
        showToast('🔋 Batarya %100 dolu!', 'success');
    ...
    EventBus.on('command:error', (d) => {
        clearCommandHighlight();
        showToast(`Komut ${d.index + 1}: ${d.message}`, 'error');
...
function renderLoop() {
    ...
    if (robotMesh) {
        // Antenna pulse
        const ab = robotMesh.children.find(c => c instanceof THREE.Mesh && c.position.y > 1.1);
        if (ab)
            ab.material.emissiveIntensity = 2 + Math.sin(time * 3) * 0.8;
        // Idle bob
        if (!isExecuting)
            robotMesh.position.y = Math.sin(time * 1.5) * 0.01;
    }
    if (robotChargeRingMat) {
...
const garageModeSelect = document.getElementById('garage-mode-select');
if (garageModeSelect) {
    garageModeSelect.value = garageMode;
    garageModeSelect.addEventListener('change', () => {
        const nextMode = garageModeSelect.value === 'closed' ? 'closed' : 'open';
...
document.getElementById('btn-green-flag').addEventListener('click', async () => {
    if (isExecuting) {
        showToast('Zaten çalışıyor!', 'warning');
        return;
    }
    if (gameState === GameState.INTRO) {
        showToast('Önce göreve başlayın!', 'warning');
        return;
    }
    const cmds = executor.getCommands().length;
    if (cmds === 0) {
        showToast('Blok ekleyin!', 'warning');
        return;
    }
    ...
    else if (!result.success && gameState !== GameState.FAILED) {
