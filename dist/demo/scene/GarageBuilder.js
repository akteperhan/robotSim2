...
export function createGarage(config) {
    ...
    g.add(slab);
    const perimeterLane = new THREE.Mesh(new THREE.BoxGeometry(garageW + 0.15, 0.04, sideDepth - 0.2), new THREE.MeshStandardMaterial({ color: 0x515767, roughness: 0.7, metalness: 0.2 }));
    ...
    for (let x = 0; x < config.GRID_W; x++) {
        for (let z = 0; z < config.GARAGE_DEPTH; z++) {
            const tile = new THREE.Mesh(new THREE.BoxGeometry(0.96, 0.04, 0.96), new THREE.MeshStandardMaterial({ color: 0x595f71, roughness: 0.79, metalness: 0.12 }));
            tile.position.set(x, -0.02, z);
            tile.receiveShadow = true;
            g.add(tile);
            if (x === config.GRID_CENTER_X || x === config.GRID_CENTER_X - 1 || x === config.GRID_CENTER_X + 1) {
                const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.16, 0.018, 0.82), new THREE.MeshStandardMaterial({ color: 0x26c6da, emissive: 0x26c6da, emissiveIntensity: 0.25, roughness: 0.35 }));
    ...
    const backLightBar = new THREE.Mesh(new THREE.BoxGeometry(garageW - 1.1, 0.06, 0.05), new THREE.MeshStandardMaterial({ color: 0x80d8ff, emissive: 0x80d8ff, emissiveIntensity: 0.8, roughness: 0.25 }));
    ...
    for (const lz of lightZ) {
        for (const lx of lightX) {
            const housing = new THREE.Mesh(new THREE.BoxGeometry(0.56, 0.07, 0.16), new THREE.MeshStandardMaterial({ color: 0xaab3c7, roughness: 0.5, metalness: 0.4 }));
            housing.position.set(lx, config.WALL_H - 0.14, lz);
            g.add(housing);
            const tube = new THREE.Mesh(new THREE.BoxGeometry(0.46, 0.025, 0.065), new THREE.MeshStandardMaterial({ color: 0xffffff, emissive: 0xe8f6ff, emissiveIntensity: 2.2, roughness: 0.1 }));
    ...
    const bench = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.75, 1.2), new THREE.MeshStandardMaterial({ color: 0x4f585f, roughness: 0.7, metalness: 0.3 }));
    bench.position.set(config.GRID_W - 0.55, 0.53, 2.2);
    bench.castShadow = true;
    g.add(bench);
    const toolCabinet = new THREE.Mesh(new RoundedBoxGeometry(0.5, 0.9, 0.42, 4, 0.04), new THREE.MeshStandardMaterial({ color: 0xcf3a3a, roughness: 0.35, metalness: 0.5 }));
    toolCabinet.position.set(config.GRID_W - 0.8, 0.46, 1.3);
    toolCabinet.castShadow = true;
    g.add(toolCabinet);
    // Roof
    const garageRoofGroup = new THREE.Group();
    const garageRoofMaterials = [];
...
//# sourceMappingURL=GarageBuilder.js.map